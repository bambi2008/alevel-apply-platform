...
export function ObjectiveExamRunner({ paper }: { paper: ObjectivePaper }) {
  ...
  const summary = summarizeModule(currentModule.questions.map((question) => question.id), answers, flagged);
  ...
  if (reviewing) return (
    <main className="mx-auto max-w-3xl px-4 py-8">
      ...
      <div className="mt-6 grid grid-cols-6 gap-2 sm:grid-cols-10">{currentModule.questions.map((question, index) => <QuestionNumber key={question.id} index={index} active={false} answered={Boolean(answers[question.id])} flagged={Boolean(flagged[question.id])} onClick={() => { setQuestionIndex(index); setReviewing(false); }} />)}</div>
...
function ObjectiveResults({ paper, answers, behavior, startedAt, timeUsedSec }: { paper: ObjectivePaper; answers: Record<string, string>; behavior: Record<string, QuestionTelemetrySnapshot>; startedAt: number; timeUsedSec: number }) {
  ...
  const moduleScores = paper.modules.map((module) => ({ module, earned: module.questions.filter((question) => answers[question.id] === question.answer).length, max: module.questions.length }));
  ...
  useEffect(() => {
    ...
    fetch("/api/exam-sessions", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({
      ...
      answers: questions.map((question) => ({ questionId: question.id, type: "mcq", selected: answers[question.id], earned: answers[question.id] === question.answer ? 1 : 0, max: 1, ...behavior[question.id] })),
    }) }).catch(() => {});
  ...
  return <main className="mx-auto max-w-3xl px-4 py-10">
    ...
    <div className="mt-3 space-y-5">{paper.modules.map((module) => <section key={module.id}><p className="mb-2 text-xs font-semibold text-neutral-500">{module.title}</p><div className="divide-y divide-neutral-100 border-y border-neutral-200">{module.questions.map((question, index) => { const selected = answers[question.id]; const correct = selected === question.answer; return <div key={question.id}><button type="button" onClick={() => setOpen((current) => ({ ...current, [question.id]: !current[question.id] }))} className="flex w-full items-center gap-3 py-3 text-left"><span className={`flex size-6 shrink-0 items-center justify-center rounded-full text-xs font-bold ${correct ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>{correct ? "✓" : "×"}</span><span className="text-xs text-neutral-400">Q{index + 1}</span><MathRenderer text={question.question} className="line-clamp-1 flex-1 text-sm" /><span className="text-xs text-neutral-400">{open[question.id] ? "收起" : "查看"}</span></button>{open[question.id] && <div className="pb-4 pl-9"><p className="text-xs text-neutral-500">你的答案：{selected || "未答"} · 正确答案：{question.answer}</p><MathRenderer text={question.solution} className="mt-2 rounded bg-neutral-50 px-3 py-2 text-sm text-neutral-700" block /></div>}</div>; })}</div></section>)}</div>

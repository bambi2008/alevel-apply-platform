...
export type ProjectField = "MECH" | "AERO" | "CS" | "ECON" | "BIO" | "MATH" | "BUS";
...
export const PROJECT_FIELDS: { value: ProjectField; label: string; labelEn: string; emoji: string }[] = [
  ...
  { value: "BUS", label: "商科/商业管理", labelEn: "Business & Management", emoji: "📈" },
];
...
export const ONLINE_PROJECTS: OnlineProject[] = [
  // ═══════════════ 机械工程 ═══════════════
  {
    ...
    reflection: [
      "这次仿真结果里，最让你意外的是什么？为什么？",
      "你的优化在工程上意味着什么权衡（成本、可制造性、安全冗余）？",
      "如果有更强算力或更多时间，你想进一步研究这个结构的哪个方面？",
      "做完后，你对'结构工程师如何做决策'有了什么新理解？",
    ],
    psUse:
      "可作为个人陈述中'用工业软件解决真实工程问题'的核心例证，展示你对结构分析的主动探索与工程判断；港校面试可讲述你的优化决策过程。",
  ...
  // ═══════════════ 航空航天工程 ═══════════════
  {
    ...
    reflection: [
      "CFD 结果和你做之前的直觉一致吗？哪里不一致，为什么？",
      "你选的湍流模型/网格如何影响结果可信度？这让你对'仿真的局限'有什么认识？",
      "最佳攻角的结论对真实飞行器设计意味着什么？",
      "你还想探索这个翼型的哪些方面（三维效应、不同雷诺数）？",
    ],
    psUse:
  ...
  // ═══════════════ 机械工程（新增） ═══════════════
  {
    ...
    reflection: [
      "疲劳失效为什么比静力失效更隐蔽、更危险？这次分析改变了你对'安全'的理解吗？",
      "你的几何优化在延长寿命的同时牺牲了什么？",
      "如果这个零件用在飞机/汽车上，你的分析还需考虑什么？",
      "这次经历让你对材料与失效有了什么新问题想深究？",
    ],
    psUse:
      "展示你理解机械工程的进阶概念（疲劳）并能做量化工程权衡；是申请机械/材料专业时区别于普通申请者的深度素材。",
  ...
  // ═══════════════ 航空航天工程（新增） ═══════════════
  {
    ...
    reflection: [
      "你的模型做了哪些简化？这些简化在什么情况下会失效？",
      "有阻力时最优发射角为什么偏离 45°？这个结果让你思考了什么？",
      "编程实现中最难的部分是什么？你如何解决的？",
      "要让模型更真实，你下一步会加入什么（风、级间分离）？",
    ],
    psUse:
  ...
  // ═══════════════ 机械工程（新增 · 热仿真） ═══════════════
  {
    ...
    reflection: [
      "散热优化中，'更大散热面积'和'体积/成本'之间你如何权衡？",
      "这次热仿真让你对身边哪些设备（手机、电脑、电动车）的设计有了新理解？",
      "忽略辐射、简化对流对你的结论影响有多大？",
      "你还想探索哪种散热方式（液冷、热管）？",
    ],
    psUse:
  ...
  // ═══════════════ 航空航天工程（新增 · 结构 FEA） ═══════════════
  {
    ...
    reflection: [
      "把'气动升力'转化为'结构载荷'的过程，让你对航空工程的系统性有什么体会？",
      "减重与强度的权衡中，你的取舍依据是什么？",
      "均布载荷 vs 椭圆载荷的简化会怎样影响你的结论？",
      "你想如何把这个课题和翼型 CFD 课题联动，做一个更完整的机翼设计？",
    ],
    psUse:
  ...
  // ═══════════════ 计算机科学（样板） ═══════════════
  {
    ...
    reflection: [
      "你的模型在哪些样本上出错？这告诉你数据或模型的什么问题？",
      "准确率高就代表模型好吗？这次经历如何改变你对'评估'的理解？",
      "如果这个模型用于真实决策（医疗、信贷），会有什么伦理风险？",
      "你还想尝试哪些方法（深度学习、更多特征）来改进？",
    ],
    psUse:
  ...
  // ═══════════════ 经济学（样板） ═══════════════
  {
    ...
    reflection: [
      "你的回归结果支持最初的假设吗？意外的发现是什么？",
      "'相关不等于因果'——这次分析中你最担心哪个遗漏变量？",
      "如果你是政策制定者，敢基于这个结果做决策吗？为什么？",
      "你想用什么更好的方法（自然实验、面板数据）加强因果推断？",
    ],
    psUse:
  ...
  // ═══════════════ 生物科学（样板） ═══════════════
  {
    ...
    reflection: [
      "跨物种的序列相似度，让你对进化与生命的共通性有什么新认识？",
      "BLAST 的 E-value 和相似度在哪些情况下会误导人？",
      "这次分析让你对'不进实验室也能做生物研究'有什么体会？",
      "你还想研究哪个基因/蛋白或哪个生物学问题？",
    ],
    psUse:
      "生物信息学在生物/医学申请中少见又有分量；个人陈述中可展示你掌握现代生物学的计算方法与数据解读能力。",
  ...
  // ═══════════════ 数学（样板） ═══════════════
  {
    ...
    reflection: [
      "从'发现规律'到'严格证明'，最难的一步是什么？",
      "你的证明/模型有哪些假设？什么情况下会不成立？",
      "这次探究让你对'数学研究'和'解题'的区别有什么体会？",
      "你想把这个结论推广到哪个更一般的情形？",
    ],
    psUse:
  ...
  // ═══════════════ 商科/商业管理（样板） ═══════════════
  {
    id: "bus-data-driven-research",
    field: "BUS",
    title: "数据驱动的商业研究：一个真实问题的量化分析",
    titleEn: "A Data-Driven Business Research Project",
    summary:
      "像研究者一样，围绕一个真实的商业/金融问题，用公开数据 + Excel（可选 Python）做量化分析，得出有证据支撑的结论——而不是写一份空泛的商业计划书。",
    scenario:
    ...
    targetMajors: ["商业管理", "金融 / 会计", "商业分析", "经济学", "市场营销"],
    difficulty: 3,
    estimatedHours: "25–40 小时（约 4–6 周）",
    primaryTool: {
      name: "Excel / Google Sheets（免费，商科最通用工具）",
      note: "Excel 是商业世界最真实、最普适的分析工具，人人可及。用它做数据整理、图表、相关与趋势分析、简单回归（数据分析工具库）。Google Sheets 免费在线，功能足够。",
    },
    altTool: {
      name: "Python (pandas)（进阶可选）",
      note: "若想处理更大数据集或更复杂分析，可用免费的 Colab + pandas；但本课题用 Excel 完全能达标。",
    },
    techStack: ["Excel 数据分析", "商业/金融研究", "数据可视化", "Python（进阶可选）"],
    baseline:
    ...
    reflection: [
      "你的分析结果支持你最初的猜想吗？最出乎意料的发现是什么？",
      "数据背后可能藏着哪些你没考虑到的因素（如季节性、外部事件）？这如何影响你的结论？",
      "如果你是这家公司的决策者，你敢基于这个分析做决定吗？还缺什么证据？",
      "这次研究让你对「用数据理解商业」有了什么新认识？你还想接着研究什么？",
    ],
    psUse:
      "个人陈述中可展示你像研究者一样用真实数据回答商业问题，体现量化能力与商业洞察；港校面试时可讲述你的研究问题、方法与结论，远比「我对商业感兴趣」有说服力。",
    skills: ["数据分析", "商业/金融研究", "Excel 建模", "批判性思维", "研究报告写作"],
    stages: [
      {
        num: 1,
        title: "提出研究问题与找数据",
        goal: "把好奇心变成一个可用数据回答的具体问题，并找到公开数据。",
        software: "公开数据源（公司财报 / Yahoo Finance / 世界银行 / Statista 等）",
        learn: [
          "好的商业研究问题：具体、可量化、能用数据回答（不是「星巴克怎么成功的」这种大而空）。",
          "真实案例参考：用疫情数据预测某公司股价、宏观指标对股市的影响、评估某公司的市场低效——都是「一个具体问题 + 数据」。",
          "公开数据来源：上市公司财报（年报/季报）、Yahoo Finance（股价）、世界银行/Statista（行业与宏观数据）。",
        ],
        task:
          "确定一个具体的商业/金融研究问题（明确自变量与因变量或对比对象），说明为什么它有意思、能用什么数据回答，并获取相关公开数据。",
        deliverable: "研究问题 + 为什么重要 + 数据来源与变量说明。",
        metrics: ["问题具体、可用数据回答", "数据真实公开可溯源", "变量/对比对象明确"],
        rubric: [
          "研究问题具体而非空泛",
          "问题有商业意义",
          "数据来源可靠、可追溯",
      ...
      },
      {
        num: 2,
        title: "数据整理与探索性分析",
        goal: "清洗数据，用图表和描述性分析看出规律。",
        software: "Excel / Google Sheets",
        learn: [
          "数据清洗：处理缺失、统一格式、剔除异常值。",
          "描述性分析：均值/增长率/占比；用折线、柱状、散点图看趋势与关系。",
          "相关分析：用相关系数看两个变量是否同向变动（但相关不等于因果）。",
        ],
        task:
          "在 Excel 中清洗数据，制作合适的图表呈现趋势与关系，计算关键描述性指标与相关系数，初步回答你的研究问题。",
        deliverable: "清洗后的数据 + 关键图表 + 描述性分析与初步发现。",
        metrics: ["数据清洗到位", "图表恰当、能说明问题", "算出关键指标与相关性"],
        rubric: [
          "数据处理规范",
          "图表清晰、服务于研究问题",
          "描述性分析正确",
      ...
      },
      {
        num: 3,
        title: "深入量化与结论（可用回归）",
        goal: "用更严谨的方法验证，得出有据的结论。",
        software: "Excel（数据分析工具库）/ Python（可选）",
        learn: [
          "简单线性回归：量化一个变量对另一个的影响（Excel 的「数据分析」工具库可做），看斜率与 R²。",
          "多因素与对比：尽量控制其它因素（如同行业对比、多期对比）以增强说服力。",
          "相关 vs 因果：数据能证明相关，但因果要非常谨慎——识别可能的干扰因素。",
        ],
        task:
          "用回归或结构化对比深入验证你的问题，量化关系强度；审慎讨论结论的可信度、相关与因果、以及数据局限。得出一个有证据支撑、有分寸的结论。",
        deliverable: "回归/深入分析结果 + 明确结论 + 因果与局限讨论。",
        metrics: ["用了更严谨的量化方法", "结论有数据支撑", "对相关/因果与局限有审慎讨论"],
        rubric: [
          "量化方法运用正确",
          "结论有分寸、不过度声称",
          "达到基础线；尽力冲击挑战目标",
      ...
      },
      {
        ...
        goal: "整合为一份规范的商业研究报告。",
        learn: [
          "报告结构：摘要 → 研究问题与背景 → 数据与方法 → 分析与发现 → 讨论与局限 → 结论。",
          "商业报告的专业性：图表规范、数字带单位与来源、结论对应证据。",
          "严谨性：说明数据来源、假设、局限（样本、时间范围、干扰因素）。",
        ],
        task: "把研究问题、数据、分析、结论整合为一份完整商业研究报告（建议 6–10 页），突出问题、方法与有证据的结论。",
        deliverable: "最终商业研究报告（PDF），含图表、量化分析、结论与局限反思。",
        metrics: ["报告含完整研究链路", "关键结论有数据/图表支撑"],
        rubric: [
          "报告结构完整、专业",
          "图表规范、被正文引用",
          "包含数据与因果局限的反思",
  ...
  {
    id: "bus-company-case-strategy",
    field: "BUS",
    title: "真实公司案例分析与战略评估",
    titleEn: "Company Case Study & Strategic Analysis",
    summary:
      "选一家真实上市公司，用经典商业框架（波特五力、SWOT）+ 财报数据，系统分析它的处境、找出问题、提出有依据的战略建议。",
    scenario:
      "咨询顾问和商业分析师每天做的事，就是「看懂一家公司、诊断问题、给出建议」。你将选一家你感兴趣的真实上市公司（如一家你了解的消费、科技或零售公司），用商业分析的经典框架（波特五力分析行业竞争、SWOT 分析优劣势）结合它公开的财报数据，系统评估它的战略处境，识别它面临的核心挑战，并提出有数据和逻辑支撑的战略建议。",
    whyValuable:
      "案例分析是商学院最核心的训练方式。「我用波特五力和财报数据分析了某公司的战略处境，识别出它的核心挑战并提出建议」——展示你掌握真实的商业分析框架、能读懂财报、有结构化的战略思维。这比空泛地说「我想学商科」有分量得多，也贴近商科大学第一年就会学的内容。",
    targetMajors: ["商业管理", "商业分析", "市场营销", "金融 / 会计", "经济学"],
    difficulty: 2,
    estimatedHours: "20–35 小时（约 3–5 周）",
    primaryTool: {
      name: "Excel / Google Sheets + 公司公开财报（免费）",
      note: "用 Excel 整理与分析公司财报数据（营收、利润、增长、关键比率）；用经典商业框架做结构化分析。所有上市公司财报公开可查（公司投资者关系页 / 交易所网站）。",
    },
    altTool: {
      name: "公司年报 + 行业报告（免费资料）",
      note: "上市公司年报（Annual Report）、行业分析文章是主要信息来源，均可免费获取。",
    },
    techStack: ["商业分析框架（波特五力/SWOT）", "财报分析", "Excel", "战略思维"],
    baseline:
      "基础线：选定一家真实上市公司，完成波特五力或 SWOT 分析，并用财报数据算出关键财务比率（如营收增长、利润率），据此描述它的战略处境。",
    challenge:
      "挑战目标：结合多年财报做趋势分析、与竞争对手做量化对比，识别核心战略挑战，并提出有数据和逻辑支撑、可执行的战略建议。",
    reflection: [
      "分析完这家公司后，你的看法和分析前有什么不同？哪个发现最意外？",
      "商业框架（五力/SWOT）帮你看清了什么？又可能让你忽略了什么？",
      "你的战略建议，最大的风险或反对意见会是什么？",
      "这次分析让你对「如何评估一家公司」有了什么新理解？你想进一步研究它的哪个方面？",
    ],
    psUse:
      "个人陈述中可展示你掌握真实商业分析框架、能读财报、有结构化战略思维；港校面试时可讲述你对某公司的诊断与建议，体现商业成熟度，是商科申请的有力素材。",
    skills: ["商业分析框架", "财报阅读与分析", "战略思维", "Excel", "商业报告写作"],
    stages: [
      {
        num: 1,
        title: "选公司与行业背景",
        goal: "选定一家真实上市公司，理解它的业务与所在行业。",
        software: "公司官网 / 年报 / 行业资料",
        learn: [
          "怎么选：选一家业务你能理解、财报公开、行业有看点的上市公司。",
          "读懂业务：它靠什么赚钱（商业模式）、主要产品、主要市场与竞争对手。",
          "信息来源：公司年报（Annual Report）、投资者关系页、行业新闻。",
        ],
        task:
          "选定一家上市公司，梳理它的商业模式、主要业务、市场与主要竞争对手，说明你为什么选它。",
        deliverable: "公司与行业背景介绍 + 选择理由。",
        metrics: ["公司真实、财报公开", "商业模式描述准确", "识别出主要竞争对手"],
        rubric: [
          "选题合理、信息可靠",
          "对业务理解准确",
          "行业背景清楚",
      ...
      {
        num: 2,
        title: "框架分析（波特五力 / SWOT）",
        goal: "用经典框架结构化分析公司的竞争处境。",
        software: "波特五力 / SWOT 框架 + 资料",
        learn: [
          "波特五力：现有竞争、新进入者威胁、替代品、供应商议价力、买方议价力——分析行业吸引力与竞争强度。",
          "SWOT：优势、劣势、机会、威胁——但要有证据，不能空泛罗列。",
          "关键：每一点都要有事实/数据支撑，避免「感觉」。",
        ],
        task:
          "对你的公司做波特五力分析（或 SWOT），每一项都用具体事实或数据支撑，得出对它竞争处境的结构化判断。",
        deliverable: "波特五力 / SWOT 分析 + 每项的证据支撑。",
        metrics: ["框架运用正确", "每项有事实/数据支撑", "得出结构化判断"],
        rubric: [
          "框架理解与运用正确",
          "分析有证据、不空泛",
          "逻辑清晰",
      ...
      },
      {
        num: 3,
        title: "财报数据分析与战略建议",
        goal: "用财报数据支撑分析，识别挑战并提建议。",
        software: "Excel + 公司财报",
        learn: [
          "关键财务比率：营收增长率、毛利率/净利率、（进阶）ROE、负债率——反映公司健康度。",
          "趋势与对比：用多年数据看趋势、与竞争对手对比，让判断有量化依据。",
          "从分析到建议：识别核心战略挑战，提出有数据和逻辑支撑、可执行的建议。",
        ],
        task:
          "用 Excel 整理公司财报，计算关键财务比率，做趋势分析与竞争对手对比；结合框架分析识别核心战略挑战，提出有依据的战略建议。",
        deliverable: "财务比率表/图 + 核心挑战识别 + 战略建议。",
        metrics: ["关键财务比率计算正确", "有趋势或对比分析", "建议有数据和逻辑支撑"],
        rubric: [
          "财报数据处理正确",
          "分析有量化依据",
          "达到基础线；尽力冲击挑战目标",
      ...
      },
      {
        num: 4,
        title: "案例分析报告",
        goal: "整合为一份专业的公司案例分析报告。",
        learn: [
          "报告结构：摘要 → 公司与行业 → 框架分析 → 财务分析 → 核心挑战 → 战略建议 → 局限 → 结论。",
          "专业性：像咨询报告一样，结论对应证据，图表规范。",
          "严谨性：说明信息来源、分析局限（如仅用公开数据、时间范围）。",
        ],
        task: "把背景、框架分析、财务分析、战略建议整合为一份完整案例分析报告（建议 6–10 页）。",
        deliverable: "最终公司案例分析报告（PDF），含框架分析、财务图表、战略建议与局限反思。",
        metrics: ["报告含完整分析链路", "建议有框架与财务数据支撑"],
        rubric: [
          "报告结构完整、专业",
          "框架与财务分析规范、被正文引用",
          "包含分析局限的反思",
    ...
    outcome:
      "一份真实公司的案例分析报告（框架分析 + 财报数据 + 战略建议）。展示你掌握商学院核心的分析框架与财报阅读能力，是商科/管理/金融申请的有力素材。",
  },

  // ═══════════════ 计算机科学（加厚 · 软件工程） ═══════════════
  {
    id: "cs-fullstack-webapp",
    field: "CS",
    title: "从零构建一个全栈 Web 应用",
    titleEn: "Building a Full-Stack Web Application from Scratch",
    summary:
      "亲手做一个真正能用的 Web 应用——从界面到后端逻辑到数据存储，部署上线并放到 GitHub 展示。不是训练模型，而是构建一个产品。",
    scenario:
      "计算机科学不只是算法，更是「构建能解决问题的软件」。你将选一个真实的小需求（如一个学习工具、习惯追踪器、小型信息管理应用），从零构建一个全栈 Web 应用：设计界面、写前端交互、做后端逻辑与数据存储，最后部署上线拿到公开链接，并把代码整理到 GitHub。这正是顶尖项目里 CS 学生真实做过的——例如「把编程基础做成 Duolingo 式学习 App」「让算法学习游戏化的 Web 应用」。",
    whyValuable:
      "一个真正能用、能展示的软件产品，是 CS 申请最有力的证据。「我从零构建并部署了一个全栈 Web 应用，代码在 GitHub」——招生官（和未来雇主）能直接看到你会构建东西、懂工程流程、能独立完成一个完整项目，远胜于只说「我喜欢编程」。",
    targetMajors: ["计算机科学", "软件工程", "信息系统", "人机交互 / 设计"],
    difficulty: 3,
    estimatedHours: "30–50 小时（约 5–7 周）",
    primaryTool: {
      name: "Python (Flask) 或 JavaScript + GitHub（免费）",
      note: "后端可用 Python Flask 或 Node.js；前端 HTML/CSS/JS。代码托管在 GitHub，用免费平台（如 Render、Vercel、PythonAnywhere）部署上线。全部免费。",
    },
    altTool: {
      name: "纯前端 (HTML/CSS/JS) + GitHub Pages（更易上手）",
      note: "若暂时不做后端，可先做一个纯前端应用（数据存浏览器），用免费的 GitHub Pages 部署，同样能展示构建能力。",
    },
    techStack: ["全栈开发", "Web 框架", "数据库", "Git / 部署"],
    baseline:
      "基础线：构建一个功能完整、能在本地运行的 Web 应用（有界面 + 核心交互逻辑），代码规范地放在 GitHub 上。",
    challenge:
      "挑战目标：加入数据库持久化、用户系统或第三方 API，部署上线拿到公开可访问链接，并为核心逻辑写测试。",
    reflection: [
      "构建过程中最难的技术问题是什么？你是怎么解决的（查文档、拆问题、调试）？",
      "如果重做一次，你会在架构或技术选型上做什么不同的决定？为什么？",
      "你的应用还有哪些没解决的问题或可改进之处？真实产品和「作业」的差距在哪？",
      "这次从零构建的经历，让你对「软件工程」和「写代码」的区别有了什么新理解？",
    ],
    psUse:
      "个人陈述中可展示你能独立构建并交付一个完整软件产品，附上 GitHub 链接就是最直接的证据；港校面试时可演示你的应用、讲述技术决策，体现真实工程能力。",
    skills: ["全栈开发", "软件工程", "Git 与部署", "问题解决", "技术文档"],
    stages: [
      {
        num: 1,
        title: "需求与设计",
        goal: "确定应用要解决的问题，规划功能与结构。",
        software: "纸笔 / 设计草图 + GitHub",
        learn: [
          "全栈的含义：前端（用户看到的界面）+ 后端（服务器逻辑）+ 数据（存储）。",
          "选题：解决一个你真实想要的小需求（工具、学习、记录类），别贪大。",
          "把想法拆成「用户故事」与功能清单；画简单的界面草图与结构图。",
          "Git / GitHub 基础：版本控制、提交、仓库。",
        ],
        task:
          "确定你的应用要解决什么问题，列出功能清单，画界面草图与简单架构，选定技术栈，建立 GitHub 仓库。",
        deliverable: "需求说明 + 功能清单 + 界面/架构草图 + GitHub 仓库。",
        metrics: ["需求明确、聚焦", "功能清单具体可实现", "GitHub 仓库已建立"],
        rubric: [
          "选题合理、范围可控",
          "功能规划清晰",
          "技术选型合理",
      ...
      },
      {
        num: 2,
        title: "前端与核心功能",
        goal: "实现界面与核心交互，让应用能用。",
        software: "HTML / CSS / JavaScript（或前端框架）",
        learn: [
        ...
        task:
          "实现应用的界面与核心交互功能，让它在本地可以运行、可以完成主要任务。",
        deliverable: "可运行的前端 + 截图 + 提交到仓库的代码。",
        metrics: ["界面可用", "核心功能实现", "代码已提交到 GitHub"],
        rubric: [
          "界面清晰、可交互",
          "核心功能可用",
          "代码结构合理",
      ...
      },
      {
        num: 3,
        title: "后端、数据与部署",
        goal: "完善后端逻辑、数据存储，并部署上线。",
        software: "Flask / Node.js + 数据库 + 免费部署平台",
        learn: [
          "后端逻辑与 API：前端如何和服务器通信。",
          "数据持久化：用数据库（或文件）保存数据。",
          "部署上线：用免费平台把应用发布到公网，拿到链接。",
          "第三方 API：让应用接入外部数据/功能（进阶）。",
        ],
        task:
          "基础：完善后端逻辑让应用完整。挑战：接入数据库/用户系统/第三方 API，部署上线拿到公开链接，并为核心逻辑写测试。",
        deliverable: "完整应用 + （挑战）公开访问链接 + 基本测试。",
        metrics: ["功能完整可用", "（挑战）已部署、公网可访问", "（挑战）有数据持久化/测试"],
        rubric: [
          "后端逻辑正确",
          "数据处理合理",
          "达到基础线；尽力冲击挑战目标（部署上线）",
      ...
      },
      {
        ...
        software: "GitHub",
        learn: [
          "GitHub 展示：README 讲清「解决什么问题、有什么功能、用了什么技术、如何运行」。",
          "代码整洁与注释，方便别人（和招生官）看懂。",
          "用截图/GIF 演示应用效果。",
        ],
        task: "把 GitHub 仓库整理成一个可展示的作品：完善 README、清理代码、加演示截图/GIF 与项目说明。",
        deliverable: "可展示的 GitHub 仓库（含完整 README 与演示）+ 项目说明。",
        metrics: ["README 完整清晰", "代码整洁、可复现", "有演示截图/GIF"],
        rubric: [
          "README 讲清了项目全貌",
          "代码整洁、可运行",
          "有直观的演示",
    ...
    outcome:
      "一个真正能用、部署上线并放在 GitHub 展示的全栈 Web 应用。是 CS/软件工程申请里最直接有力的「我能构建东西」的证据，可写进 PS 并附链接。",
  },

  // ═══════════════ 计算机科学（加厚 · 算法与理论） ═══════════════
  {
    id: "cs-algorithms-analysis",
    field: "CS",
    title: "算法的实现、可视化与复杂度分析",
    titleEn: "Implementing, Visualising & Analysing Algorithms",
    summary:
      "亲手实现一组经典算法，验证正确性，实测不同规模输入的运行时间，把理论复杂度和实测数据对照——触摸计算机科学的理论内核。",
    scenario:
    ...
    targetMajors: ["计算机科学", "数学与计算机", "软件工程", "数据科学"],
    difficulty: 3,
    estimatedHours: "25–40 小时（约 4–6 周）",
    primaryTool: {
      name: "Python (matplotlib) + GitHub（免费）",
      note: "用任意语言实现算法（Python 上手快），用 matplotlib 画复杂度曲线，代码放 GitHub。可用免费的 Colab 运行。",
    },
    altTool: {
      name: "任意编程语言 + Desmos（备选）",
      note: "算法可用你熟悉的任何语言实现；复杂度曲线也可用 Desmos/表格软件绘制。",
    },
    techStack: ["算法与数据结构", "复杂度分析 (Big-O)", "Python", "数据可视化"],
    baseline:
      "基础线：正确实现一组算法（如多种排序，或搜索/图算法），验证正确性，对不同规模输入实测运行时间，画出并解释其时间复杂度。",
    challenge:
      "挑战目标：对比多种算法、用曲线拟合把实测数据和理论 O(...) 对照、分析最坏/平均情形，或做算法的可视化动画。",
    reflection: [
      "你的实测运行时间和课本的理论复杂度一致吗？哪里有偏差，你觉得为什么？",
      "在什么情况下，一个「更差复杂度」的算法反而可能更快？这让你对「效率」有什么新认识？",
      "实现算法时，哪个细节最容易出错？这让你对「正确性」有什么体会？",
      "这次分析让你对「计算机科学是一门什么样的学科」有了什么新理解？你还想深入哪个算法领域？",
    ],
    psUse:
      "个人陈述中可展示你理解算法效率、能做理论与实测对照的严谨分析——这正是英国 CS（牛剑）最看重的算法与数学思维；代码放 GitHub 可作为佐证。",
    skills: ["算法与数据结构", "复杂度分析", "Python 编程", "实验与数据分析", "技术写作"],
    stages: [
      {
        num: 1,
        title: "选题与算法背景",
        goal: "选定一组可对比的算法，理解其理论复杂度。",
        software: "纸笔 + GitHub",
        learn: [
          "时间复杂度与 Big-O：算法运行时间如何随输入规模增长（O(n)、O(n log n)、O(n²) 等）。",
          "为什么算法效率至关重要：数据一大，慢算法就不可用。",
          "选一组可对比的算法：如排序（冒泡/归并/快排）、搜索、或图算法（BFS/DFS）。",
          "相关数据结构基础。",
        ],
        task:
          "选定一组算法，说明它们解决什么问题、各自的理论时间复杂度与预期差异，建立 GitHub 仓库。",
        deliverable: "算法选择说明 + 理论复杂度 + GitHub 仓库。",
        metrics: ["算法选择合理、可对比", "理论复杂度说明正确", "仓库已建立"],
        rubric: [
          "理解时间复杂度概念",
          "算法选择有对比价值",
          "理论分析正确",
      ...
      },
      {
        ...
        title: "实现与正确性验证",
        goal: "用代码实现算法并验证其正确。",
        software: "Python（或任意语言）",
        learn: [
          "把算法逻辑准确翻译成代码。",
          "测试正确性：用多组输入（含边界情形）验证。",
          "代码规范与可读性。",
        ],
        task:
          "实现你选定的算法，用测试用例（含边界与特殊情形）验证它们的正确性。",
        deliverable: "算法代码 + 测试用例 + 正确性说明。",
        metrics: ["算法实现正确", "通过测试用例", "代码已提交到 GitHub"],
        rubric: [
          "算法实现无误",
          "测试覆盖边界情形",
          "代码清晰、有注释",
      ...
      },
      {
        ...
        learn: [
          "实测：对不同规模的输入计时（注意多次取平均、控制变量）。",
          "画「输入规模—运行时间」曲线；观察增长趋势。",
          "理论 vs 实测：验证曲线形状是否符合 Big-O；可做曲线拟合。",
        ],
        task:
          "对一系列递增规模的输入实测运行时间，画出复杂度曲线，把实测和理论复杂度对照分析。挑战：拟合曲线、对比多种算法、分析最坏/平均情形。",
        deliverable: "运行时间数据 + 复杂度曲线图 + 理论与实测对照分析。",
        metrics: ["实测数据规模充分", "曲线正确反映增长趋势", "理论与实测对照到位"],
        rubric: [
          "实测方法合理（多次、控制变量）",
          "曲线绘制正确",
          "达到基础线；尽力冲击挑战目标",
      ...
      },
      {
        num: 4,
        title: "报告与展示",
        goal: "整合为一份算法分析报告 / GitHub 展示。",
        software: "GitHub / 文档",
        learn: [
          "结构：算法原理 → 实现 → 复杂度实测与分析 → 结论。",
          "GitHub README 或报告，把过程和发现讲清楚。",
          "图表规范：复杂度曲线有坐标、图例、单位。",
        ],
        task: "把算法实现、复杂度分析整合为一份报告或可展示的 GitHub 仓库，含算法说明、代码、复杂度曲线与结论。",
        deliverable: "算法分析报告（或 GitHub 仓库，含 README、代码、曲线图）。",
        metrics: ["含完整分析链路", "复杂度分析严谨", "图表规范"],
        rubric: [
          "报告/README 结构完整",
          "复杂度分析严谨、有理论与实测对照",
          "图表规范、被引用",
  ...
  // ═══════════════ 经济学（加厚 · 预测建模） ═══════════════
  {
    id: "econ-forecasting",
    field: "ECON",
    title: "经济预测：用数据找出预测某变量的关键因素",
    titleEn: "Economic Forecasting: Predicting a Variable from Data",
    summary:
      "选一个可预测的经济变量（如房价、门票价、失业率），用公开数据建立预测模型，找出关键预测因素并评估准确度——像经济分析师一样用数据预测。",
    scenario:
    ...
    targetMajors: ["经济学", "计量经济学 / 金融", "数据科学（社科方向）", "商业分析"],
    difficulty: 3,
    estimatedHours: "25–40 小时（约 4–6 周）",
    primaryTool: {
      name: "Python (pandas + scikit-learn / statsmodels)（免费）",
      note: "建预测模型的主流工具，免费，可用 Colab 在线运行。数据来自世界银行、FRED、Kaggle、各类公开数据集。",
    },
    altTool: {
      name: "Excel / R（备选）",
      note: "Excel 可做回归预测（数据分析工具库）；R 是经济学界常用。均可完成本课题。",
    },
    techStack: ["预测建模", "回归 / 机器学习", "Python", "数据可视化"],
    baseline:
    ...
    reflection: [
      "哪个预测因素出乎你的意料？这对你理解这个经济现象有什么启发？",
      "「能预测」和「能解释因果」是两回事——你的模型属于哪种？这个区别为什么重要？",
      "如果用你的模型做真实决策，最大的风险是什么（如数据变化、外部冲击）？",
      "这次建模让你对「用数据预测经济」有了什么新认识？你还想预测什么？",
    ],
    psUse:
      "个人陈述中可展示你能把经济问题转化为数据预测模型、并严谨评估——体现计量与数据分析能力；港校面试可讲述你的预测问题、方法与发现，远胜空谈兴趣。",
    skills: ["预测建模", "回归 / 机器学习", "数据分析", "经济学思维", "研究报告写作"],
    stages: [
      {
        num: 1,
        title: "预测问题与数据",
        goal: "确定要预测的变量与候选预测因素，找到数据。",
        software: "公开数据源（世界银行 / FRED / Kaggle 等）",
        learn: [
          "预测 vs 因果：预测只需找出「能预测」的因素，不必证明因果。",
          "选一个可预测变量：房价、门票价、某商品价、失业率、消费等。",
          "找候选预测因素（自变量）：哪些数据可能与目标相关。",
          "公开数据来源与获取。",
        ],
        task:
          "确定预测目标（因变量）与一组候选预测因素（自变量），说明为什么它们可能相关，获取公开数据。",
        deliverable: "预测问题 + 候选变量 + 数据来源说明。",
        metrics: ["预测目标明确、可量化", "候选因素合理", "数据真实公开"],
        rubric: [
          "预测问题有意义",
          "候选因素有经济逻辑",
          "数据来源可靠",
      ...
      },
      {
        num: 2,
        title: "数据探索与特征选择",
        goal: "清洗数据，探索各因素与目标的关系。",
        software: "Python (pandas) / Excel",
        learn: [
          "探索性分析：各因素与目标的相关性、散点关系。",
          "特征选择：留下真正有预测力的变量。",
          "数据清洗与预处理。",
        ],
        task:
          "清洗数据，用图表和相关分析探索各候选因素与目标变量的关系，选定进入模型的特征。",
        deliverable: "数据探索图表 + 特征选择说明。",
        metrics: ["数据清洗到位", "探索了因素与目标的关系", "特征选择有依据"],
        rubric: [
          "数据处理规范",
          "探索充分、有可视化",
          "特征选择合理",
      ...
      },
      {
        ...
        title: "建模与评估",
        goal: "建立预测模型并严谨评估准确度。",
        software: "Python (scikit-learn / statsmodels) / Excel",
        learn: [
          "回归预测模型；正确划分训练/测试集评估泛化能力。",
          "预测误差指标：RMSE、MAE；拟合优度 R²。",
          "过拟合的识别；（挑战）对比多种模型、时间序列或样本外验证。",
        ],
        task:
          "建立预测模型，在测试集上评估预测误差，报告关键预测因素与准确度。挑战：对比多种模型、做样本外/时间序列验证、分析特征重要性。",
        deliverable: "预测模型 + 误差指标 + 关键预测因素 + （挑战）模型对比。",
        metrics: ["正确划分训练/测试", "报告 RMSE/MAE 与 R²", "识别关键预测因素", "（挑战）多模型对比"],
        rubric: [
          "建模方法正确",
          "评估严谨、防数据泄露",
          "达到基础线；尽力冲击挑战目标",
      ...
      },
      {
        ...
        goal: "整合为一份规范的预测研究报告。",
        learn: [
          "报告结构：问题 → 数据 → 方法 → 模型与结果 → 讨论与局限 → 结论。",
          "严谨性：说明预测 vs 因果、数据与模型局限、不确定性。",
          "图表规范：预测结果、特征重要性有说明。",
        ],
        task: "把问题、数据、建模、评估整合为一份完整预测研究报告（建议 6–10 页）。",
        deliverable: "最终预测研究报告（PDF），含模型、误差、关键因素与局限反思。",
        metrics: ["含完整预测研究链路", "关键结论有指标支撑"],
        rubric: [
          "报告结构完整、专业",
          "图表规范、被正文引用",
          "包含预测局限与不确定性的反思",
  ...
  // ═══════════════ 经济学（加厚 · 行为经济学实验） ═══════════════
  {
    id: "econ-behavioral-experiment",
    field: "ECON",
    title: "行为经济学：设计实验验证一个决策偏差",
    titleEn: "Behavioural Economics: Designing an Experiment to Test a Bias",
    summary:
      "选一个经典的行为经济学效应（如锚定、损失厌恶、从众），自己设计问卷/实验、收集真实数据、做统计检验——这是少见的、能自己产生一手数据的研究课题。",
    scenario:
      "行为经济学揭示：人并非完全理性，决策会被心理因素系统性影响。你将选一个经典效应（如锚定效应、损失厌恶、框架效应、从众），像研究者一样设计一个规范的问卷或小实验，向真实人群（如同学、网络）收集数据，用统计方法检验你是否观察到了这个效应。这是所有课题里**唯一能让你自己产生一手研究数据**的方向，充分展示研究设计能力。",
    whyValuable:
      "自己设计实验、收集一手数据、做统计检验——这是最接近真实学术研究的经历。「我设计实验检验了锚定效应，收集了 N 份真实数据并做了显著性检验」——招生官会看到你懂研究设计、能处理真实数据、理解行为经济学前沿，这在申请者中非常稀缺。",
    targetMajors: ["经济学", "行为经济学 / 心理学", "商业（消费者行为）", "公共政策"],
    difficulty: 3,
    estimatedHours: "25–40 小时（约 4–6 周）",
    primaryTool: {
      name: "问卷工具（Google Forms，免费）+ Excel / Python 做统计",
      note: "用免费的 Google Forms 设计并发放实验/问卷，收集数据；用 Excel 或 Python 做描述统计与显著性检验。全部免费、人人可及。",
    },
    altTool: {
      name: "纸质问卷 + Excel（备选）",
      note: "若线上不便，可用纸质问卷线下收集，Excel 完成分析。核心是研究设计的严谨，不是工具。",
    },
    techStack: ["行为经济学", "实验 / 调研设计", "统计检验", "数据分析"],
    baseline:
      "基础线：选定一个行为经济学效应，设计一个规范的问卷/实验，收集真实数据，分析是否观察到该效应，并做基本的描述统计与对比。",
    challenge:
      "挑战目标：设置对照组/控制变量、做统计显著性检验（如 t 检验、卡方）、报告效应大小，并审慎讨论样本局限与内外部效度。",
    reflection: [
      "你的实验结果验证了这个效应吗？如果没有，你觉得是效应不存在，还是实验设计的问题？",
      "你的样本（如同学）有哪些局限？这如何影响结论能推广到多大范围？",
      "设计实验时，你如何避免「引导受试者」或其它偏差？这让你对「客观研究」有什么体会？",
      "这次亲手做研究的经历，让你对经济学（尤其行为经济学）有了什么新理解？",
    ],
    psUse:
      "个人陈述中可展示你能独立设计实验、收集一手数据、做统计分析——这是极稀缺的真实研究经历；港校面试可讲述你的研究设计与发现，体现研究潜质。",
    skills: ["实验 / 调研设计", "行为经济学", "统计检验", "数据分析", "研究报告写作"],
    stages: [
      {
        num: 1,
        title: "选效应与实验设计",
        goal: "选一个行为经济学效应，设计规范的实验/问卷。",
        software: "文献阅读 + 实验设计",
        learn: [
          "行为经济学核心：人非完全理性，存在系统性偏差。",
          "经典效应：锚定效应、损失厌恶、框架效应、从众、禀赋效应等。",
          "实验/调研设计：假设、分组（对照）、随机化、控制变量、避免引导性问题。",
        ],
        task:
          "选一个你感兴趣的效应，明确研究假设，设计一个规范的实验或问卷方案（如何分组、如何测量、如何避免偏差）。",
        deliverable: "选定效应 + 研究假设 + 实验/问卷设计方案。",
        metrics: ["效应明确、有文献基础", "设计规范（有对照/控制思路）", "假设可检验"],
        rubric: [
          "理解所选效应",
          "实验设计科学、有对照意识",
          "假设清晰可检验",
      ...
      },
      {
        ...
        task:
          "制作并发放你的实验/问卷，向真实人群收集数据。注意样本量、伦理与数据质量。",
        deliverable: "问卷/实验材料 + 收集到的真实数据。",
        metrics: ["样本量足够分析", "数据真实、来自真实受试者", "符合基本研究伦理"],
        rubric: [
          "问卷/实验实施规范",
          "样本量与质量可用",
          "遵守研究伦理",
      ...
      },
      {
        num: 3,
        title: "分析与统计检验",
        goal: "分析数据，检验是否观察到该效应。",
        software: "Excel / Python",
        learn: [
          "描述统计：均值、分布、分组对比。",
          "统计显著性检验：t 检验、卡方检验的基本概念与使用。",
          "效应大小；相关不等于因果；结果解读要审慎。",
        ],
        task:
          "分析收集的数据，对比分组，做基本统计检验，判断是否观察到了目标效应。挑战：报告效应大小、控制变量、讨论显著性。",
        deliverable: "数据分析 + 统计检验结果 + 是否验证效应的结论。",
        metrics: ["描述统计正确", "做了统计显著性检验", "结论审慎、有据"],
        rubric: [
          "分析方法正确",
          "统计检验运用得当",
          "达到基础线；尽力冲击挑战目标",
      ...
      },
      {
        num: 4,
        title: "研究报告",
        goal: "整合为一份规范的行为经济学研究报告。",
        learn: [
          "报告结构：研究问题 → 假设 → 实验设计 → 数据与方法 → 结果 → 讨论与局限 → 结论。",
          "严谨性：讨论样本局限、内外部效度、可能的偏差。",
          "图表规范：分组对比、检验结果有说明。",
        ],
        task: "把假设、设计、数据、分析整合为一份完整的行为经济学研究报告（建议 6–10 页）。",
        deliverable: "最终行为经济学研究报告（PDF），含设计、数据、检验、结论与局限反思。",
        metrics: ["含完整研究链路", "结论有数据与检验支撑"],
        rubric: [
          "报告结构完整、专业",
          "实验设计与分析规范",
          "包含样本与效度局限的反思",
    ...
    outcome:
      "一份行为经济学研究报告（自主设计实验 + 一手数据 + 统计检验）。是少见的「自己做研究」经历，充分展示研究设计与数据分析能力，是经济/心理/社科申请的独特素材。",
  },

  // ═══════════════ 数学（加厚 · 计算数论 / 实验数学） ═══════════════
  {
    id: "math-computational-number-theory",
    field: "MATH",
    title: "计算数论：用编程做实验数学，发现并证明规律",
    titleEn: "Computational Number Theory: Experimental Mathematics",
    summary:
      "选一个具体而窄的数论问题（如 Collatz 序列、质数间隔），用编程计算生成大量数据、发现规律、提出猜想，再尽力证明——像数学家做实验一样。",
    scenario:
    ...
    targetMajors: ["数学 / 应用数学", "计算机科学（理论）", "统计学", "物理学"],
    difficulty: 3,
    estimatedHours: "25–40 小时（约 4–6 周）",
    primaryTool: {
      name: "Python (numpy / matplotlib) + GitHub（免费）",
      note: "用 Python 计算生成数据、找规律、作图。数据完全靠自己计算生成，无需外部数据集。可用免费 Colab。",
    },
    altTool: {
      name: "任意编程语言 + Desmos（备选）",
      note: "任何你熟悉的语言都可以做计算；Desmos/表格可辅助可视化。核心是数学探究，编程是工具。",
    },
    techStack: ["计算数论", "实验数学", "Python", "数据分析"],
    baseline:
      "基础线：选一个具体而窄的数论问题，用编程计算生成数据、发现规律并提出明确猜想，再给出证明（或严谨的统计/实证分析）。",
    challenge:
      "挑战目标：证明猜想（或严谨的部分证明）、扩大计算规模验证、做严格的统计相关性分析或反例搜索。",
    reflection: [
      "从计算数据中发现规律的那一刻，你的思路是怎样的？数据如何引导了你的猜想？",
      "计算能验证很多case，但为什么不能替代证明？这让你对「数学真理」有什么体会？",
      "如果你的猜想在更大范围出现反例，你会如何调整？",
      "这次「实验数学」的经历，让你对数学研究和数学考试的区别有什么新理解？",
    ],
    psUse:
      "个人陈述中可展示你能像研究者一样「计算—发现—猜想—证明」，并结合编程与数学严谨——这正是数学系最看重的研究潜质；代码放 GitHub 可作佐证。",
    skills: ["数学探究", "计算与编程", "证明与推理", "数据分析", "数学写作"],
    stages: [
      {
        num: 1,
        title: "选题与背景",
        goal: "选一个具体而窄、可计算的数论问题。",
        software: "文献阅读 + Python + GitHub",
        learn: [
          "实验数学：用计算发现规律，再提猜想、求证明。",
          "「具体而窄」的重要性：「质数」是一个宇宙，「1 万以内质数间隔的分布」才是一个研究问题。",
          "候选问题：Collatz 序列长度、质数间隔、数字根、某数列规律、完全数等。",
        ],
        task:
          "选一个具体而窄的数论问题，说明它为什么有意思、可以用计算探索，建立 GitHub 仓库。",
        deliverable: "研究问题 + 背景 + GitHub 仓库。",
        metrics: ["问题具体而窄（非泛泛领域）", "可用计算探索", "有数学意义"],
        rubric: [
          "选题聚焦、可执行",
          "问题有探索价值",
          "理解实验数学思路",
      ...
      },
      {
        ...
        task:
          "写程序计算大量案例，用可视化和统计探索其中的规律，提出一个明确的猜想。",
        deliverable: "计算代码 + 数据/图 + 明确的猜想。",
        metrics: ["计算规模充分", "规律清晰、有可视化", "猜想明确可检验"],
        rubric: [
          "计算正确、规模足够",
          "善用可视化发现规律",
          "猜想明确",
      ...
      },
      {
        num: 3,
        title: "证明或严谨分析",
        goal: "证明猜想，或做严谨的统计/实证分析。",
        software: "纸笔推理 + Python 验证",
        learn: [
          "能证就证：数学归纳、反证、构造等方法。",
          "不能完全证明时：做严谨的统计相关性分析、置信度、反例搜索。",
          "计算验证能增强信心，但不等于证明。",
        ],
        task:
          "对你的猜想给出严谨证明或部分证明；若难以证明，则做严格的统计/实证分析并扩大计算规模验证。诚实说明你证到了什么程度。",
        deliverable: "证明（或部分证明）或严谨统计分析 + 计算验证。",
        metrics: ["证明严谨 或 统计分析严谨", "验证充分", "对结论程度诚实"],
        rubric: [
          "证明/分析逻辑严谨",
          "方法恰当",
          "达到基础线；尽力冲击挑战目标",
      ...
      },
      {
        ...
        software: "文档 / GitHub",
        learn: [
          "结构：问题 → 计算与发现 → 猜想 → 证明/分析 → 反思 → 结论。",
          "数学写作规范：定义、猜想、证明清晰分明；符号一致。",
          "诚实呈现：说清证到什么程度、还有什么开放问题。",
        ],
        task: "把问题、计算、猜想、证明/分析整合为一份完整数学探究报告（建议 8–12 页），注重严谨与诚实。",
        deliverable: "最终数学探究报告（PDF），含计算、猜想、证明/分析与反思。",
        metrics: ["含完整探究链路", "证明/分析严谨呈现"],
        rubric: [
          "报告结构完整、专业",
          "数学表述规范、符号一致",
          "包含开放问题与延伸的反思",
  ...
  // ═══════════════ 数学（加厚 · 应用数学建模） ═══════════════
  {
    id: "math-applied-modelling",
    field: "MATH",
    title: "应用数学建模：用数学描述一个真实现象",
    titleEn: "Applied Mathematical Modelling of a Real Phenomenon",
    summary:
      "选一个真实现象（如传染病传播、人口增长、排队、游戏概率），建立数学模型，用 Python 数值求解、可视化，并用真实数据验证——数学如何解释真实世界。",
    scenario:
    ...
    targetMajors: ["数学 / 应用数学", "统计学", "工程学", "经济学 / 物理学"],
    difficulty: 3,
    estimatedHours: "25–40 小时（约 4–6 周）",
    primaryTool: {
      name: "Python (numpy / scipy / matplotlib)（免费）",
      note: "用 Python 数值求解模型（scipy 解微分方程）、可视化。可用免费 Colab。真实数据来自公开来源（如疫情、人口数据）。",
    },
    altTool: {
      name: "Excel / Desmos（备选）",
      note: "简单模型可用 Excel 迭代或 Desmos 可视化。核心是建模思路与验证，工具可灵活。",
    },
    techStack: ["数学建模", "微分方程 / 迭代", "Python", "数据可视化"],
    baseline:
      "基础线：选一个真实现象，建立数学模型（微分方程/迭代/概率），用 Python 数值求解并可视化，讨论模型行为与合理性。",
    challenge:
      "挑战目标：用真实数据拟合/验证模型参数、对比多个模型、做参数敏感性分析，并讨论模型的适用范围与局限。",
    reflection: [
      "你的模型做了哪些简化假设？在什么情况下这些假设会失效？",
      "模型和真实数据吻合吗？不吻合的地方，是模型的问题还是数据的问题？",
      "一个「简单但不完全准确」的模型，价值在哪里？这让你对「建模」有什么体会？",
      "这次建模让你对「数学如何解释真实世界」有了什么新理解？你还想建模什么现象？",
    ],
    psUse:
      "个人陈述中可展示你能把数学用于真实问题、并用数据验证——体现应用数学思维与编程能力；港校面试可讲述你的模型、假设与验证，展示学以致用。",
    skills: ["数学建模", "数值方法", "Python", "数据验证", "数学写作"],
    stages: [
      {
        num: 1,
        title: "选现象与建立模型",
        goal: "选一个真实现象，写出它的数学模型。",
        software: "文献/资料 + 建模",
        learn: [
          "数学建模：用数学（方程/规则）描述真实现象的机制。",
          "经典模型：SIR 传染病模型、Logistic 人口增长、排队论、概率模型（游戏/彩票）。",
          "建模三要素：变量、假设、关系（方程）。好模型抓住本质、合理简化。",
        ],
        task:
          "选一个真实现象，建立它的数学模型（写出方程或迭代规则），明确变量、参数与假设。",
        deliverable: "现象说明 + 数学模型（方程/规则）+ 假设。",
        metrics: ["现象适合建模", "模型合理、抓住本质", "假设明确"],
        rubric: [
          "现象选择合适",
          "模型建立合理",
          "假设清楚、有依据",
      ...
      },
      {
        num: 2,
        title: "数值求解与可视化",
        goal: "用 Python 求解模型，画出并解读结果。",
        software: "Python (scipy / matplotlib)",
        learn: [
          "数值方法：用 scipy 解微分方程（solve_ivp），或迭代计算。",
          "用 matplotlib 可视化模型行为（如疫情曲线、人口曲线）。",
          "解读模型：它预测了什么？行为是否符合直觉？",
        ],
        task:
          "用 Python 数值求解你的模型，画出结果图，解读模型的行为与含义。",
        deliverable: "求解代码 + 结果图 + 模型行为解读。",
        metrics: ["数值求解正确", "结果图合理", "解读到位"],
        rubric: [
          "数值方法运用正确",
          "可视化清晰",
          "对模型行为解读合理",
      ...
      },
      {
        ...
        learn: [
          "用真实数据验证或拟合模型参数（如用真实疫情数据拟合 SIR）。",
          "参数敏感性分析：参数变化如何影响结果。",
          "模型局限：哪些现实因素被忽略了；（挑战）对比更复杂的模型。",
        ],
        task:
          "用真实数据验证或拟合模型参数，做参数敏感性分析，讨论模型的适用范围与局限。挑战：对比多个模型。",
        deliverable: "数据验证/拟合 + 敏感性分析 + 局限讨论。",
        metrics: ["有真实数据验证或敏感性分析", "对模型局限有讨论", "（挑战）多模型对比"],
        rubric: [
          "验证/拟合方法合理",
          "敏感性分析到位",
          "达到基础线；尽力冲击挑战目标",
      ...
      },
      {
        ...
        software: "文档 / GitHub",
        learn: [
          "结构：现象 → 模型与假设 → 数值求解 → 验证与分析 → 局限 → 结论。",
          "严谨性：说清假设、简化、验证方法与局限。",
          "图表规范：模型曲线、数据对比有说明。",
        ],
        task: "把现象、模型、求解、验证整合为一份完整数学建模报告（建议 8–12 页）。",
        deliverable: "最终数学建模报告（PDF），含模型、求解图、数据验证与局限反思。",
        metrics: ["含完整建模链路", "关键结论有图表/数据支撑"],
        rubric: [
          "报告结构完整、专业",
          "图表规范、被正文引用",
          "包含假设与局限的反思",
